...
function storeCards() {
    Cards.map(card => AsyncStorage.setItem(card.id, JSON.stringify(card.image)));
    this.setState({ card: JSON.stringify(this.card.image) });
}

function getCards() {
    AsyncStorage.getItem(card.id).then(JSON.stringify(card.image) => {
           this.setState({card: JSON.stringify(this.card.image)});
       })
}
